...
func (pq *PaymentQuery) QueryTrip() *TripQuery {
...
func (pq *PaymentQuery) WithTrip(opts ...func(*TripQuery)) *PaymentQuery {
	query := (&TripClient{config: pq.config}).Query()
...
func (pq *PaymentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Payment, error) {
	...
	if query := pq.withTrip; query != nil {
		if err := pq.loadTrip(ctx, query, nodes, nil,
			func(n *Payment, e *Trip) { n.Edges.Trip = e }); err != nil {
			return nil, err
		}
	}
...
func (pq *PaymentQuery) loadTrip(ctx context.Context, query *TripQuery, nodes []*Payment, init func(*Payment), assign func(*Payment, *Trip)) error {
...
func (pq *PaymentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := pq.ctx.Fields; len(fields) > 0 {
		...
		if pq.withTrip != nil {
			_spec.Node.AddColumnOnce(payment.FieldTripID)
		}
		if pq.withUser != nil {
